...
(function() {
    ...
    this.createWorker = function(session) {
        var worker = new WorkerClient(["ace"], "ace/worker/my-worker", "MyWorker", "http://10.151.44.20/elearning/public/js/my-worker.js");
        worker.attachToDocument(session.getDocument());

        worker.on("annotate", function(results) {
            var editorstatustext = document.getElementById("editorstatustext");
            editorstatustext.style.backgroundColor = "green";
            editorstatustext.innerHTML = "Checker Status : IDLE";
            session.setAnnotations(results.data);
            var timerstart = document.getElementById("timer");
            var timerend = new Date().getTime();
            var time = timerend - timerstart.value;
            // alert('Checking Time: ' + time);
            softSubmit();
        });
